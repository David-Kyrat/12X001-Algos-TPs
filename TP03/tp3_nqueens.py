...
def solve_bt(n:int) -> list[list[int]]:
...
def compare_naive_and_backtracking():
    ...
    max_n = 5
    plot_n = [n for n in range(4, max_n+2)]
...
def plot_bt():
    max_n = 10
    plot_n = [n for n in range(4, max_n+1)]
    plot_f2 = runtime_arr(solve_bt, plot_n, unpack=False)
    xlabel = "Chessboard Dimension"
    ylabel = "Runtime (s)"
    title = "Runtime of Backtracking solution"
    f2label = "Backtracking implementation"
    plot_solo(plot_n, plot_f2, title, xlabel, ylabel, f2label)
