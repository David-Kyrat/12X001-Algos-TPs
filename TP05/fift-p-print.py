...
def c_hat(board: list[list[int]]) -> int:
    """Cost function for a given board is the cost function the optimum, i.e. ĉ(board) = ĉ(optimum) = ĉ(root)
    (by relation 3.3 in the lecture notes). 
    and for root, ĉ(root) = h(root) + g(root) = 0 + g(root) = g(root) = number of misplaced tiles (16 excluded).
    Hence why we only compute g(root) here.
...
def nextENode(liveNodes: list[Node]) -> Node:
    ...
    @ Returns: ``liveNodes.pop()`` i.e. Node with that minimize the cost ĉ in given liveNodes list.
    """
...
def solve_taquin(board: list[list[int]], extract_path_from_goalNode: bool = True, white_square:int = 16) -> list[str] | Node:
    ...
    @ `convert_sol` - If True, return the list of moves as string (i.e. 'up', 'down'...) to get from the initial state to the goal state.
    If False, return the goal node.
    @ `white_square` - (only useful if board is a nxn matrix with n>4) Value of the white square in the initial state of the game.
    Default is 16. NB: does not modify given board """
